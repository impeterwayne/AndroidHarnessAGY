...
the screen that already exists, the `core/designsystem` component that already matches the
design's button, the drawable already in `res/drawable/`, the `AppTheme` token that already
holds that colour.
...
1. **Text** — `scan_text_nodes` with `{ nodeId, depth: 4 }`. Split static from dynamic, record `fontSize`, `fontWeight`, `lineHeight`, colour, and the `AppTheme.typography` style each maps to. Add a `cd_` row for every icon and image. → §7
...
3. **Tokens** — `export_tokens` / `get_styles` / `get_variable_defs`. Map each value to an `AppTheme` token. Where none exists, record the nearest with its delta — that cell is the input to stage 2's snap decision, not a formality. → §6
...
Stage 3 builds a preview per row. A state absent from this table is a state that ships
unimplemented.
