...
3. **Reuse First**: Check for existing helper methods, base classes (`BaseActivity`, `BaseFragment`, `BaseViewModel`), shared layouts and custom views in `core/ui`, `Widget.App.*` styles, `@color`/`@dimen`/`TextAppearance.App.*` resources, and the existing Action/SideEffect patterns before writing new ones.
...
5. **Platform & Library First**: Use ShapeView attributes, `ConstraintLayout`, VectorDrawables, Glide, Epoxy and native platform features before reaching for another external dependency.
