...
- **No hardcoded user-facing strings** — `res/values/strings.xml`, then `@string/…` in the
  layout or `getString(...)` in Kotlin.
- **No raw hex colours** — `@color/…`. The one exception is `colors.xml` itself.
- **No new `<shape>` / `<selector>` / `<ripple>` drawable** — fills, corners, borders,
  gradients, state colours, ripples and shadows are ShapeView `app:shape_*` attributes.
- **Lists are Epoxy** — a controller and `@EpoxyModelClass` models, never a hand-written
  `RecyclerView.Adapter` and never `notifyDataSetChanged`.
- **Reuse first** — shared layouts, custom views and styles in `core/ui`, Kotlin
  stdlib/KTX, existing use cases and repositories before writing anything new.
- **Preserve the contracts** — Clean Architecture boundaries, the Action/SideEffect
  contract, existing ViewModel and navigation signatures, and the `android:id` values a
  binding already exposes. Never break a caller to make your change convenient.
