...
| :--- | :--- |
| Kotlin symbols, class and function names | `grep_search` on the identifier |
| Module layout, source-set structure | `list_dir`, `settings.gradle.kts` |
| Which screens use a layout or a custom view | `grep_search` on the layout name, the binding class, or the view's class name |
...
`core/ui` (shared layouts, custom views, `styles.xml`) and `res/values` before concluding
that a component, style or resource does not exist.
