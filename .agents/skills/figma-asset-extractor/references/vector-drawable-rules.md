...
- **Target Directories**:
  - App-wide common icons: `core/designsystem/src/main/res/drawable/` or `app/src/main/res/drawable/`.
...
| `tint` | Unset / `#FF...` | Leave unset if the icon will be tinted dynamically in Compose (`tint = AppTheme.colorScheme.colorText` or `tint = Color(...)`). Set only if the icon has a permanent hardcoded tint. |
...
## 5. Jetpack Compose Image & Icon Rendering

- **Vector Icons**: Use `Icon(painter = painterResource(id = R.drawable.ic_*), ...)` or `Image(painter = painterResource(id = R.drawable.ic_*), ...)`.
- **Dynamic Images**: Use Skydoves Landscapist `GlideImage` (`com.skydoves.landscapist.glide.GlideImage`) with `imageModel`, `previewPlaceholder`, `loading`, and `failure` composables.
